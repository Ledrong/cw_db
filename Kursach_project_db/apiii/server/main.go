...
func main() {
	...
	viper.SetConfigName("setting")                                                              // имя файла без расширения
	viper.AddConfigPath("C:\\Users\\ledron\\GolandProjects\\Kursach_project_db\\apiii\\server") // путь к файлу (текущий каталог)
	viper.SetConfigType("json")                                                                 // тип файла конфигурации
	err := viper.ReadInConfig()
	if err != nil {
		return
	}

	// прочитать настройки из файла конфигурации в структуру setting
	cfg := new(setting)
	...
	db, err := sqlx.Open("postgres", fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=disable", cfg.PgHost, cfg.PgPort, cfg.PgUser, cfg.PgPassword, cfg.PgBase))
	if err != nil {
		return
	}
	defer db.Close()

	// создать новую реализацию интерфейса CPURepository с использованием подключения к БД
	...
	// создать новую интеракторную логику с использованием CPURepository и RAmRepository
	...
	// следить за изменениями файла конфигурации
	viper.WatchConfig()
	viper.OnConfigChange(func(e fsnotify.Event) {
		...
		// закрыть все соединения и пересоздать DB с новыми параметрами:
		db.Close()
		db, err = sqlx.Open("postgres", fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=disable",
			cfg.PgHost, cfg.PgPort, cfg.PgUser, cfg.PgPassword, cfg.PgBase))
		if err != nil {
			fmt.Printf("failed to open database connection: %s\n", err)
		}

		// обновить вашу реализацию интеракторной логики с использованием новых настроек

	})

	// определить маршрут для выполнения HTTP-запроса через ваше приложение echo
	...
	// Main-горутина продолжает работать и ожидает сигнал выхода
	<-make(chan os.Signal, 1)

	// после завершения работы сервера закрыть все соединения БД
}
...
//_ = viper.New()
//viper.SetConfigName("setting") // имя файла без расширения
//viper.AddConfigPath(".")       // путь к файлу (текущий каталог)
//viper.SetConfigType("json")    // тип файла конфигурации
//err := viper.ReadInConfig()
//if err != nil {
//	return
...
// запустите сервер:
//log.Println("start server in :5432")
//err := srv.ListenAndServe()
//if err != nil {
//	e.Logger.Fatal(err)
//}
